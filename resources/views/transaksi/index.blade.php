...
<body>
    ...
    <div class="d-flex justify-content-center mt-5">
        <div class="shadow p-3 bg-body rounded">
                ...
                <tbody>
                    @foreach($transaksis as $transaksi)
                        <tr>
                            ...
                            <td>{{$transaksi->jenis_kacamata}}</td>
                            <td>Rp. {{number_format("$transaksi->harga_kacamata",2)}}</td>
                            <td>{{date("d F Y", strtotime($transaksi->tanggal_transaksi))}}</td>
                            <td>
                                <a href="ubah/{{$transaksi->id_transaksi}}" class="btn btn-warning">
                                    UBAH
                                </a>
                                <a href="hapus/{{$transaksi->id_transaksi}}" class="btn btn-danger">
                                    HAPUS
                                </a>
                            </td>
                        </tr>
            ...
            <a href="tambah/{{$customer->id_customer}}" class="btn btn-primary mt-3 w-100">TAMBAH</a>
        </div>
    </div>
    <script>
